...
const Story:React.FC<StoryProps> = ({img,name,profile}) => {
    ...
    return (
        <div className="rounded-2xl w-[7rem] h-[13rem] bg-[#e2dddd] cursor-pointer relative 
         bg-no-repeat bg-cover bg-center transition-all hover:scale-[1.05]" style={{backgroundImage: `url(${img})`}}>
           <img className="absolute top-1 left-2 bg-[red] w-10 h-10 rounded-full" src={profile} alt="profile img" />
