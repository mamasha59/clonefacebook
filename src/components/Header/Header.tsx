...
const Header:React.FC = () => {
    ...
    <header className="flex px-5 justify-between items-center shadow-xl bg-[#fff] sticky top-0 z-10 lit:py-1">
        {/* logo-input block */}
        <div className="flex items-center flex-[1_1_20%] relative xl:flex-row sm:flex-1">
            <div className="flex items-center grow">
                <div className={`w-9 h-9 mr-3 flex items-center ${active ? 'z-30': ''}`}>
                ...
                <label className={`flex border-solid border bg-[#f0f2f5] rounded-3xl w-[100%] xl:w-[auto] xl:p-[9px]`}>
                    ...
                    <input onClick={makeActive} className={`input-style transition-all xl:absolute xl:top-0 ${active ? 'z-30 xl:block': ''}` }type="text" placeholder="Поиск на Facebook" />
                </label>
        ...
        <div className={`transition-transform translate-x-[-100%] text-center bg-[#fff] shadow-lg pt-[5%] px-2 pb-2 w-[22%] shad absolute top-0 left-0 rounded-md xl:w-[27%] xl:pt-[6%] ${active?'translate-x-0':''}`}><p>No results</p></div>
    </header>
    );
